...
class MYSQL:
    ...
    def __init__(self, dbhost, dbport, dbuser, dbpwd, dbname, dbcharset):
        self.dbhost = dbhost
        self.dbport = dbport
        self.dbuser = dbuser
        self.dbpwd = dbpwd
    ...
    def fetch_one(self, table, fields=None, condition=None, order=None):
        return self.fetch_rows(self, table, fields, condition, order, '1', True)

    def query(self, sql, condition=None, fetchone=False):
    ...
    def execute(self, sql, condition=None):
        """execute custom sql"""
        with self.connection.cursor() as cursor:
            if not sql:
                return
            if condition:
                ...
                cursor.execute(sql)
            return self.connection.commit()  # not auto commit

    def join_field_value(self, data, glue=', '):
        sql = comma = ''
        for key in data.keys():
            sql += "{0}`{1}` = %s".format(comma, key)
            comma = glue
        return sql
